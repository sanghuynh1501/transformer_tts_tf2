...
class HDF5DatasetWriter:
    def __init__(self, audio_dims, label_dims, output_path, batch_size=8, buffer_size=10):
        ...
        self.stop_tokens = self.db.create_dataset("stop_tokens", (audio_dims[0], audio_dims[1]), dtype="int",
                                                  compression="gzip", chunks=(batch_size, audio_dims[1]))

        self.bufSize = buffer_size
        self.buffer = {"audios": [], "labels": [], "audio_lens": [], "label_lens": [], "stop_tokens": []}
        self.idx = 0

    def add(self, audios, labels, audio_lens, label_lens, stop_tokens):
        self.buffer["audios"].extend(audios)
        self.buffer["labels"].extend(labels)
        self.buffer["audio_lens"].extend(audio_lens)
        self.buffer["label_lens"].extend(label_lens)
        self.buffer["stop_tokens"].extend(stop_tokens)
    ...
    def flush(self):
        ...
        self.stop_tokens[self.idx:i] = self.buffer["stop_tokens"]
        self.idx = i
        self.buffer = {"audios": [], "labels": [], "audio_lens": [], "label_lens": [], "stop_tokens": []}
...
class HDF5DatasetGenerator:
    ...
    def generator(self):
        ...
        for i in range(0,  len(self.indexes), self.batchSize):
            ...
            stop_tokens = self.db["stop_tokens"][self.indexes[i]: self.indexes[i] + self.batchSize]
            yield np.array(audios), np.array(labels), np.array(audio_lens), np.array(label_lens), np.array(stop_tokens)
